...
doFolds folds points = foldl' (flip doFold) points folds

showPoint folded mx (x,y)
  | (x,y) `elem` folded = '#'
  | x > mx = '\n'
  | otherwise = ' '
...
main = do
  ...
  let folded = doFolds folds points
      (mx,my) = (maximum $ map fst folded, maximum $ map snd folded)
  putStr [showPoint folded mx (x,y) | y <- [0..my], x <- [0..mx+1]]
